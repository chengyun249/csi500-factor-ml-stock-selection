...
def build_readme(top50: pd.DataFrame, top100: pd.DataFrame, model_ic: pd.DataFrame) -> str:
    ...
    readme = (
        ...
        "```\n"
        "This target is the cross-sectional rank of the return from the stated execution date "
        "to the next execution date, exactly matching the portfolio holding period.\n"
        "\n"
        ...
        "LightGBM has a higher mean test Rank IC than Ridge in the original factor space, "
        "but its cost-adjusted Top50 portfolio is worse and it does not outperform the low-volatility single-factor strategy. "
        ...
        "\n"
        "All 14 reviewed Top50 variants remain `exploratory_research_only`: the locked window has only "
        "23 complete months. The block-bootstrap audit and pre-declared continuation gates are in "
        "[`reports/final/strategy_acceptance_review.md`](reports/final/strategy_acceptance_review.md). "
        "The two low-volatility variants have positive unadjusted 95% lower bounds, but their "
        "multiple-comparison familywise lower bounds are negative and neither meets the 36-month gate.\n"
        "\n"
        "## 9. Limitations\n"
        "\n"
        "```text\n"
        ...
        "\n"
        "> A reproducible CSI500 cross-sectional research workflow in which low volatility is the "
        "most promising candidate, while the current sample is insufficient to establish a deployable edge "
        "and machine learning adds no reliable portfolio-level improvement.\n"
    )
...
def build_report(top50: pd.DataFrame, top100: pd.DataFrame, model_ic: pd.DataFrame, factor_ic: pd.DataFrame) -> str:
    ...
    report = (
        ...
        + top100_table + "\n"
        "\n"
        "结果显示，原始低波动 Top50 是收益最强的策略，行业中性低波动次之。"
        "所有 LightGBM Top50 组合在 15bp 成本后的绝对年化收益均为负；原始 LightGBM 还明显跑输同期全收益基准。"
        "行业中性化后，低波动仍保留较强表现，但 LightGBM 组合进一步下降。"
